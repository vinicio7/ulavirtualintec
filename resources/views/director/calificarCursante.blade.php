...
@section('content')
    ...
    <div class="row border-bottom white-bg dashboard-header">
        ...
        {!! Form::open(['route' => 'director.formCalifCursante', 'method' => 'POST'])!!}
        ...
        <div class="" style="text-align:center; margin-bottom: 15px;">
            ...
            <select class="selectpicker" name="cursante" data-live-search="true">
                @foreach($cursantes as $c)
                    <option>{{($c -> nombres . ' ' . $c -> paterno . ' ' . $c -> materno . ' - ' .$c -> id)}}</option>;
                @endforeach
            </select>
        </div>
        ...
        <p align="center">
            <button type="submit" class=" btn-primary btn-sm glyphicon glyphicon-check" style="text-align:center" name="materia" value=<?php echo($materiaId) ?>> ENVIAR CALIFICACIÓN</button>
        </p>
        {!! Form::close() !!}
    </div>
